...
inline void setTensorNdDesc(cudnnTensorDescriptor_t* desc,
  ...
  if(shape.size() <= 4) {
	  int shapeBuf[4] = {1, 1, 1, 1};
	  int strideBuf[4] = {1, 1, 1, 1};
	  for(int i = 0; i < shape.size(); ++i) {
		  shapeBuf[i] = shape[i];
		  strideBuf[i] = stride[i];
	  }

	  CUDNN_CHECK(cudnnSetTensorNdDescriptor(*desc, dataType<Dtype>::type,
			4, shapeBuf, strideBuf));
  } else {
...
/* for ND support */
template <typename Dtype>
inline void createConvolutionNdDesc(cudnnConvolutionDescriptor_t* conv) {
  CUDNN_CHECK(cudnnCreateConvolutionDescriptor(conv));
}
/**/
...
inline void setConvolutionNdDesc(cudnnConvolutionDescriptor_t* conv,
	...
	std::vector<int>dilation(pad.size(), 1);
#if CUDNN_VERSION_MIN(6, 0, 0)
  CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv,
  		pad.size(), pad.data(), stride.data(), dilation.data(), CUDNN_CROSS_CORRELATION,
      dataType<Dtype>::type));
#else
    CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv,
    		pad.size(), pad.data(), stride.data(), dilation.data(), CUDNN_CROSS_CORRELATION));
...
inline void createPoolingNdDesc(cudnnPoolingDescriptor_t* pool_desc,
  ...
  CUDNN_CHECK(cudnnSetPoolingNdDescriptor(*pool_desc, *mode,
        CUDNN_PROPAGATE_NAN, shape.size(), shape.data(), pad.data(), stride.data()));
#else
  CUDNN_CHECK(cudnnSetPooling2dDescriptor_v4(*pool_desc, *mode,
        CUDNN_PROPAGATE_NAN, shape.size(), shape.data(), pad.data(), stride.data()));
#endif
