...
/* for ND support */
template <typename Dtype>
__global__ void LRNFillScale_ND(const int nthreads, const Dtype* const in,
    const int num_spatial_axes,
    int* const shape,
    const int size, const Dtype alpha_over_size,
    const Dtype k, Dtype* const scale) {

  int* axis = (int*)malloc((num_spatial_axes + 2)*sizeof(int));
  int volume_size = 1; for(int dim = 0; dim < (num_spatial_axes + 2); ++dim) {
    volume_size *= shape[dim];
  }

  CUDA_KERNEL_LOOP(index, nthreads) {
    // find out the local offset
    for(int dim = 0, volume = volume_size, residual = index; dim < (num_spatial_axes + 2); ++dim) {
      volume /= shape[dim];
      axis[dim] = residual / volume;
      residual %= volume;
    }
    ...
    const int h = (index / width) % height;
    const int n = index / width / height;
    cinst int offset = n * channels * height * width
           + c * height * width
           + h * width
           + w;
    const int step = height * width;
    */
    ...
    int offset = 0;
    axis[1] = 0; // start from the first channel of each image
    for (int dim = 0, volume = volume_size; dim < num_spatial_axes + 2; ++dim) {
      volume /= shape[dim];
      offset += axis[dim] * volume;
    }

    int step = 1;
    for (int dim = 2; dim < num_spatial_axes + 2; ++dim) {
      step *= shape[dim];
    }
...
void LRNLayer<Dtype>::CrossChannelForward_gpu(
  ...
  // We will launch one kernel for each pixel location, and have the kernel
  // go through all the channels.
  int n_threads = this->shape_[0];
  for (int dim = 2; dim < shape_.size(); ++dim) {
    n_threads *= this->shape_[dim];
  }
  // NOLINT_NEXT_LINE(whitespace/operators)
  int* shape_d;
  CUDA_CHECK(cudaMalloc((void**)&shape_d, shape_.size() * sizeof(int)));
  caffe_gpu_memcpy(shape_.size() * sizeof(int), &shape_[0], shape_d);

  LRNFillScale_ND<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, bottom_data,
      shape_.size()-2,
      shape_d,
      size_,
      alpha_ / size_, k_, scale_data);
...
/* for ND support */
template <typename Dtype>
__global__ void LRNComputeDiff_ND(const int nthreads,
    const Dtype* const bottom_data, const Dtype* const top_data,
    const Dtype* const scale, const Dtype* const top_diff,
    const int num_spatial_axes,
    int* const shape,
    const int size, const Dtype negative_beta,
    const Dtype cache_ratio, Dtype* const bottom_diff) {
...
void LRNLayer<Dtype>::CrossChannelBackward_gpu(
    ...
    const vector<Blob<Dtype>*>& bottom) {
  int n_threads = this->shape_[0];
  for(int dim = 2; dim < this->shape_.size(); ++dim) {
    n_threads *= this->shape_[dim];
  }
  // NOLINT_NEXT_LINE(whitespace/operators)
  int* shape_d;
  ...
  LRNComputeDiff_ND<<<CAFFE_GET_BLOCKS(n_threads), CAFFE_CUDA_NUM_THREADS>>>(
      n_threads, bottom[0]->gpu_data(), top[0]->gpu_data(),
      scale_.gpu_data(), top[0]->gpu_diff(),
      this->shape_.size() - 2,
      shape_d,
      size_, -beta_, Dtype(2. * alpha_ * beta_ / size_),
      bottom[0]->mutable_gpu_diff());
